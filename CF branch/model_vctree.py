...
class VCTreeLSTMContext(nn.Module):
    ...
    def __init__(self, config, obj_classes, rel_classes, statistics,in_channels):
        ...
        if self.effect_analysis:
            ...
            self.register_buffer("untreated_avg_edg_encod", torch.zeros([self.num_obj_classes,(self.hidden_dim + self.obj_dim)]))
    ...
    def obj_ctx(self, num_objs, obj_feats, proposals, obj_labels=None, vc_forest=None, ctx_average=False):
        ...
        for i, (feat, tree, proposal) in enumerate(zip(obj_feats, vc_forest, proposals)):
            encod_rep,_ = self.obj_ctx_rnn(tree, feat, len(proposal))
    ...
    def edge_ctx(self, num_objs, obj_feats, forest,flag_edge = False,avg_edge_feat =None,obj_preds = None,obj_embed2=None):
        """
        Object context and object classification.
        :param obj_feats: [num_obj, img_dim + object embedding0 dim]
        ...
        for feat, tree, num_obj in zip(inp_feats, forest, num_objs):
            edge_rep,avg_edge_rep = self.edge_ctx_rnn(tree, feat, num_obj,flag_edge,avg_edge_feat,obj_preds,obj_embed2)
            edge_ctxs.append(edge_rep)
            avg_edge_ctxs.append(avg_edge_rep)
        edge_ctxs = cat(edge_ctxs, dim=0)
        avg_edge_ctxs = cat(avg_edge_ctxs, dim=0)
        return edge_ctxs,avg_edge_ctxs

    #def forward(self, x, proposals, rel_pair_idxs, logger=None, all_average=False, ctx_average=False):
    def forward(self, x, proposals, rel_pair_idxs,co_occour, logger=None, all_average=False, ctx_average=False):#0527
        ...
        #bi_preds, vc_scores = self.zeros_score(num_objs)
        #by zhouhao
        bi_preds, vc_scores = self.vctree_score_net(num_objs, bi_inp, obj_logits, proposals,co_occour) #0527
        ...
        avg_edg_encod = self.untreated_avg_edg_encod
        flag_edge = True
        edge_ctx,avg_edge_ctxs = self.edge_ctx(num_objs, obj_rel_rep, vc_forest,flag_edge,avg_edg_encod,obj_preds,obj_embed2)
        ...
        if self.training and self.effect_analysis:
            ...
            self.untreated_avg_edg_encod = self.moving_average_mat(self.untreated_avg_edg_encod, cat((x, obj_ctxs), -1),obj_preds)

        return obj_dists, obj_preds, edge_ctx, bi_preds,avg_edge_ctxs
    ...
    def moving_average_mat(self, holder, input, obj_preds):
        assert len(input.shape) == 2
        with torch.no_grad():
            for i_avg in range(len(obj_preds)):
                holder[obj_preds[i_avg]] = holder[obj_preds[i_avg]] * (1 - self.average_ratio) + self.average_ratio * input[i_avg]
        return holder

    def vctree_score_net(self, num_objs, roi_feat, roi_dist, proposals,co_occour):
        ...
        for sub, obj, dist, prp in zip(sub_feats, obj_feats, roi_dists, proposals):
            ...
            co_prior = torch.mm(dist,co_occour)
            co_prior = torch.mm(co_prior,dist.t())
    ...
    def zeros_score(self,num_objs):
        bi_preds = []
        vc_scores = []
        for num_bi in num_objs:
            bi_pred = torch.zeros(num_bi,num_bi).cuda()
            bi_preds.append(bi_pred)
            vc_scores.append(torch.sigmoid(bi_pred))
        return bi_preds, vc_scores
