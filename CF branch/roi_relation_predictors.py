...
class CausalAnalysisPredictor(nn.Module):
    def __init__(self, config, in_channels):
        ...
        self.co_occour = statistics['pred_dist'].float().sum(-1).cuda() #0527
        assert self.num_obj_cls==len(obj_classes)
        assert self.num_rel_cls==len(rel_classes)
        # init contextual lstm encoding
        if config.MODEL.ROI_RELATION_HEAD.CAUSAL.CONTEXT_LAYER == "motifs":
            ...
            self.context_layer_vctree = VCTreeLSTMContext(config, obj_classes, rel_classes, statistics, in_channels)
            #self.context_layer_vctree = VCTreeLSTMContext(config, obj_classes, rel_classes, in_channels)
        ...
        self.ctx_avg_fc = nn.Linear(self.num_rel_cls*2, self.num_rel_cls)
        layer_init(self.ctx_avg_fc, xavier=True)
    ...
    def pair_feature_generate(self, roi_features, proposals, rel_pair_idxs, num_objs, obj_boxs, logger, ctx_average=False):
        ...
        if self.cfg.MODEL.ROI_RELATION_HEAD.CAUSAL.CONTEXT_LAYER == "vctree":
            obj_dists, obj_preds, edge_ctx, binary_preds,avg_edge_ctxs = self.context_layer_vctree(roi_features, proposals, rel_pair_idxs, self.co_occour, logger, ctx_average=ctx_average) #0527
            #obj_dists, obj_preds, edge_ctx, binary_preds,avg_edge_ctxs = self.context_layer_vctree(roi_features, proposals, rel_pair_idxs, logger, ctx_average=ctx_average)
        else:
            obj_dists, obj_preds, edge_ctx, binary_preds,avg_edge_ctxs = self.context_layer(roi_features, proposals, rel_pair_idxs, logger, ctx_average=ctx_average)
        ...
        ##
        avg_edge_rep = self.post_emb(avg_edge_ctxs)#avg_edge_ctxs
        avg_edge_rep = avg_edge_rep.view(avg_edge_rep.size(0), 2, self.edge_dim)
        avg_head_rep = avg_edge_rep[:, 0].contiguous().view(-1, self.edge_dim)
        avg_tail_rep = avg_edge_rep[:, 1].contiguous().view(-1, self.edge_dim)
        # split
        head_reps = head_rep.split(num_objs, dim=0)
        tail_reps = tail_rep.split(num_objs, dim=0)
        ###
        avg_head_reps = avg_head_rep.split(num_objs, dim=0)
        avg_tail_reps = avg_tail_rep.split(num_objs, dim=0)
        ###
        obj_preds = obj_preds.split(num_objs, dim=0)
        ...
        pair_obj_probs = []
        pair_bboxs_info = []
        for pair_idx, head_rep, tail_rep, obj_pred, obj_box, obj_prob, avg_head_rep, avg_tail_rep in zip(rel_pair_idxs, head_reps, tail_reps, obj_preds, obj_boxs, obj_prob_list,avg_head_reps,avg_tail_reps):
            if self.use_vtranse:
                ctx_reps.append( head_rep[pair_idx[:,0]] - tail_rep[pair_idx[:,1]] )
                avg_ctx_reps.append( avg_head_rep[pair_idx[:,0]] - avg_tail_rep[pair_idx[:,1]] )
            else:
                ctx_reps.append( torch.cat((head_rep[pair_idx[:,0]], tail_rep[pair_idx[:,1]]), dim=-1) )
                avg_ctx_reps.append( torch.cat((avg_head_rep[pair_idx[:,0]], avg_tail_rep[pair_idx[:,1]]), dim=-1) )
            pair_preds.append( torch.stack((obj_pred[pair_idx[:,0]], obj_pred[pair_idx[:,1]]), dim=1) )
        ...
        avg_ctx_rep = cat(avg_ctx_reps, dim=0)
        if self.use_vtranse:
            post_ctx_rep = ctx_rep
            avg_post_ctx_rep = avg_ctx_rep
        else:
            post_ctx_rep = self.post_cat(ctx_rep)
            avg_post_ctx_rep = self.post_cat(avg_ctx_rep)

        return post_ctx_rep, pair_pred, pair_bbox, pair_obj_probs, binary_preds, obj_dist_prob, edge_rep, obj_dist_list,avg_post_ctx_rep, avg_edge_rep
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        ...
        assert len(num_rels) == len(num_objs)
        # by zhouhao
        post_ctx_rep, pair_pred, pair_bbox, pair_obj_probs, binary_preds, obj_dist_prob, edge_rep, obj_dist_list,avg_post_ctx_rep, avg_edge_rep = self.pair_feature_generate(roi_features, proposals, rel_pair_idxs, num_objs, obj_boxs, logger)

        # for avg_train and test
        #by zhouhao
        # if (not self.training) and self.effect_analysis:
        #     with torch.no_grad():
        #         avg_post_ctx_rep, _, _, avg_pair_obj_prob, _, _, _, _,_ ,_ = self.pair_feature_generate(roi_features, proposals, rel_pair_idxs, num_objs, obj_boxs, logger, ctx_average=True)

        if self.separate_spatial:
            union_features, spatial_conv_feats = union_features
            post_ctx_rep = post_ctx_rep * spatial_conv_feats
            avg_post_ctx_rep = avg_post_ctx_rep * spatial_conv_feats###
        
        if self.spatial_for_vision:
            post_ctx_rep = post_ctx_rep * self.spt_emb(pair_bbox)
            avg_post_ctx_rep = avg_post_ctx_rep* self.spt_emb(pair_bbox) ###avg

        #by zhouhao
        cal_type='sum_ori'
        rel_dists,rel_dists_diff = self.calculate_logits_avg(union_features, post_ctx_rep, pair_pred, avg_post_ctx_rep, cal_type, use_label_dist=False)
        rel_dist_list = rel_dists.split(num_rels, dim=0)
        rel_dists_diff_list = rel_dists_diff.split(num_rels, dim=0)
        # rel_dists = self.calculate_logits(union_features, post_ctx_rep, pair_pred, use_label_dist=False)
        # rel_dist_list = rel_dists.split(num_rels, dim=0)
        ###
        #avg_rel_dists = self.calculate_logits(union_features, avg_post_ctx_rep, pair_pred, use_label_dist=False)
        #avg_rel_dists = avg_rel_dists.split(num_rels, dim=0)
        ...
        if self.training:
            ...
            # binary loss for VCTree  by zhouhao
            if binary_preds is not None:
                binary_loss = []
                for bi_gt, bi_pred in zip(rel_binarys, binary_preds):
                    bi_gt = (bi_gt > 0).float()
                    binary_loss.append(F.binary_cross_entropy_with_logits(bi_pred, bi_gt))
                add_losses["binary_loss"] = 0.33*(sum(binary_loss) / len(binary_loss))

            # branch constraint: make sure each branch can predict independently by zhouhao
            add_losses['auxiliary_ctx'] = F.cross_entropy(self.ctx_compress(post_ctx_rep), rel_labels)
            add_losses['auxiliary_ctx_avg'] = F.cross_entropy(self.ctx_compress(avg_post_ctx_rep), rel_labels)
            add_losses['auxiliary_rel_dists'] = F.cross_entropy(rel_dists, rel_labels)
            ##by zhouhao test to do
            # if cal_type == 'sum' or cal_type =='sum_ori':
        ...
        return obj_dist_list, rel_dists_diff_list, add_losses
    ...
    def calculate_logits_avg(self, vis_rep, ctx_rep, frq_rep,avg_ctx_rep, cal_type, use_label_dist=True, mean_ctx=False):
        if use_label_dist:
        ...
        avg_ctx_dists = self.ctx_compress(avg_ctx_rep)

        if cal_type == 'gate':
            ctx_gate_dists = self.ctx_gate_fc(ctx_rep)
            union_dists = (ctx_dists-avg_ctx_dists) * torch.sigmoid(vis_dists + frq_dists + ctx_gate_dists)
            ...
            union_dists = self.ctx_avg_fc(cat(((ctx_dists-avg_ctx_dists),ctx_dists),dim=-1))
        elif cal_type == 'sum':
            union_dists = (ctx_dists-avg_ctx_dists) + vis_dists + frq_dists
        elif cal_type == 'diff_gate':
            ctx_gate_dists = self.ctx_gate_fc(ctx_rep)
            ctx_avg_dists = self.ctx_avg_fc(cat(((ctx_dists-avg_ctx_dists),ctx_dists),dim=-1))
            union_dists = ctx_avg_dists * torch.sigmoid(vis_dists + frq_dists + ctx_gate_dists)
        elif cal_type =='gate_ori':
            ctx_gate_dists = self.ctx_gate_fc(ctx_rep)
            union_dists = ctx_dists * torch.sigmoid(vis_dists + frq_dists + ctx_gate_dists)
        elif cal_type =='sum_ori':
            union_dists = vis_dists + ctx_dists
            #union_dists = vis_dists + ctx_dists + frq_dists
        else:
            print('invalid cal_type')
        ###by zhouhao
        #union_dists_diff = union_dists + (ctx_dists-avg_ctx_dists)
        #0829
        union_dists_diff = vis_dists + 0.5*ctx_dists + 0.5*(ctx_dists-avg_ctx_dists)

        return union_dists,union_dists_diff
