...
class MultiLayer_BTreeLSTM(nn.Module):
    ...
    def forward(self, tree, features, num_obj,flag_edge=False,avg_edge_feat=None,obj_preds = None,obj_embed2=None):
        for i in range(self.num_layer):
            features,avg_features = self.multi_layer_lstm[i](tree, features, num_obj,flag_edge,avg_edge_feat,obj_preds,obj_embed2)
        return features,avg_features
...
class BidirectionalTreeLSTM(nn.Module):
    ...
    def forward(self, tree, features, num_obj,flag_edge=False,avg_edge_feat=None,obj_preds=None,obj_embed2=None):
        foreward_output,foreward_avg_output = self.treeLSTM_foreward(tree, features, num_obj,flag_edge,avg_edge_feat,obj_preds,obj_embed2)
        backward_output,backward_avg_output = self.treeLSTM_backward(tree, features, num_obj,flag_edge,avg_edge_feat,obj_preds,obj_embed2)
    
        final_output = torch.cat((foreward_output, backward_output), 1)
        final_avg_output = None
        if (foreward_avg_output == None) or (backward_avg_output==None):
            final_avg_output = None
        else:
            final_avg_output = torch.cat((foreward_avg_output, backward_avg_output), 1)
        return final_output,final_avg_output
...
class OneDirectionalTreeLSTM(nn.Module):
    ...
    def forward(self, tree, features, num_obj,flag_edge=False,avg_edge_feat=None,obj_preds=None,obj_embed2=None):
        ...
        lstm_io = TreeLSTM_IO(None, h_order, 0, None, None, dropout_mask,None)
        # run tree lstm forward (leaves to root)
        self.treeLSTM(tree, features, lstm_io,flag_edge,avg_edge_feat,obj_preds,obj_embed2)
        # resume order to the same as input
        output = lstm_io.hidden[lstm_io.order.long()]
        avg_output =None
        if flag_edge == True:
            avg_output = lstm_io.avg_hidden[lstm_io.order.long()]
...
class BiTreeLSTM_Foreward(nn.Module):
    ...
    def forward(self, tree, features, treelstm_io,flag_edge=False,avg_edge_feat=None,obj_preds=None,obj_embed2=None):
        """
        ...
        if tree.left_child is not None:
            self.forward(tree.left_child, features, treelstm_io,flag_edge,avg_edge_feat,obj_preds,obj_embed2)
        if tree.right_child is not None:
            self.forward(tree.right_child, features, treelstm_io,flag_edge,avg_edge_feat,obj_preds,obj_embed2)
        ...
        if flag_edge == True:
            avg_feat_cls = avg_edge_feat[obj_preds[tree.index]].view(1, -1)
            avg_feat_cls = cat((obj_embed2[tree.index].view(1, -1), avg_feat_cls), -1)
            avg_c, avg_h = self.node_forward(avg_feat_cls, left_c, right_c, left_h, right_h, treelstm_io.dropout_mask)
            if treelstm_io.avg_hidden is None:
                treelstm_io.avg_hidden = avg_h.view(1, -1)
            else:
                treelstm_io.avg_hidden = cat((treelstm_io.avg_hidden, avg_h.view(1, -1)), 0)
...
class BiTreeLSTM_Backward(nn.Module):
    ...
    def forward(self, tree, features, treelstm_io,flag_edge=False,avg_edge_feat=None,obj_preds=None,obj_embed2=None):
        ...
        if flag_edge == True:
            avg_feat_cls = avg_edge_feat[obj_preds[tree.index]].view(1, -1)
            avg_feat_cls = cat((obj_embed2[tree.index].view(1, -1), avg_feat_cls), -1)
            avg_c, avg_h = self.node_backward(avg_feat_cls, root_c, root_h, treelstm_io.dropout_mask)
            if treelstm_io.avg_hidden is None:
                treelstm_io.avg_hidden = avg_h.view(1, -1)
            else:
                treelstm_io.avg_hidden = cat((treelstm_io.avg_hidden, avg_h.view(1, -1)), 0)
...
class TreeLSTM_IO(object):
    def __init__(self, hidden_tensor, order_tensor, order_count, dists_tensor, commitments_tensor, dropout_mask,avg_hidden_tensor):
        ...
        self.avg_hidden = avg_hidden_tensor