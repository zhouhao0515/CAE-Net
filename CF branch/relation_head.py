...
class ROIRelationHead(torch.nn.Module):
    ...
    def forward(self, features, proposals, targets=None, logger=None):
        ...
        loss_relation, loss_refine, loss_bf,loss_ht = self.loss_evaluator(proposals, rel_labels, relation_logits, refine_logits)
        #print(loss_relation)

        if self.cfg.MODEL.ATTRIBUTE_ON and isinstance(loss_refine, (list, tuple)):
            output_losses = dict(loss_rel=loss_relation, loss_refine_obj=loss_refine[0], loss_refine_att=loss_refine[1],loss_bf=loss_bf,loss_ht = loss_ht)
        else:
            output_losses = dict(loss_rel=loss_relation, loss_refine_obj=loss_refine,loss_bf=loss_bf,loss_ht = loss_ht)

        output_losses.update(add_losses)
        # refine_logits, relation_logits, add_losses,relation_logits_diff = self.predictor(proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger)

        # # for test
        # if not self.training:
        #     result = self.post_processor((relation_logits_diff, refine_logits), rel_pair_idxs, proposals)
        #     return roi_features, result, {}

        # loss_relation, loss_refine, loss_bf,loss_ht = self.loss_evaluator(proposals, rel_labels, relation_logits, refine_logits,relation_logits_diff)
        # #print(loss_relation)
