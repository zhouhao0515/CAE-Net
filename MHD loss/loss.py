...
class RelationLossComputation(object):
    ...
    def __init__(
        ...
        weight_bce_ht = torch.FloatTensor([1,1]).cuda()
        self.bce_loss_ht = nn.BCEWithLogitsLoss(weight = weight_bce_ht)
        ...
        head_tail = []
        with open('/home/statlab/scenegraphcode/0527exp_vg/exp1/Scene-Graph-Benchmark.pytorch-master/freq.csv', 'r') as f:
            for _, freq in enumerate(f):
                if float(freq) > self.thre:
                    head_tail.append(1)
                else:
                    head_tail.append(0)
        self.head_tail = torch.from_numpy(np.asarray(head_tail))
            

    def background_cancel(self,relation_logits,rel_labels,relation_logits_diff):
        soft_output = F.softmax(relation_logits)
        max_value = soft_output.max(axis = 1).indices
        n_rel_labels= rel_labels[torch.where(~((max_value == 0) & (max_value == rel_labels)))]
        n_relation_logits= relation_logits_diff[torch.where(~((max_value == 0) & (max_value == rel_labels)))]
    ...
    def EQLloss(self,relation_logits,rel_labels,relation_logits_diff,qbj = False):
        if qbj:
            relation_logits,rel_labels = self.background_cancel(relation_logits,rel_labels,relation_logits_diff)
    ...
    def foreloss(self,relation_logits_diff,rel_labels):
        mask_fg = torch.gt(rel_labels,0)
        rel_labels = rel_labels[mask_fg]-1
        relation_logits_diff = relation_logits_diff[mask_fg][:,1:]
        rel_loss = self.criterion_loss(relation_logits_diff,rel_labels)
        #####
        ht_labels = self.convert_label_ht(rel_labels)
        ht_relation_logits = self.convert_logits_ht(relation_logits_diff)
        ht_loss = self.bce_loss_ht(ht_relation_logits,ht_labels.float())

        return rel_loss,ht_loss
    ...
    def convert_logits(self,relation_logits):
        ...
        for i in range(len(max_value)):
            if max_value[i]>0:
                ...
                #bf_relation_logits[i,0] = 0.0 (C-1)
            else:
                bf_relation_logits[i,1] = relation_logits[i,1:-1].mean()
        return bf_relation_logits

    def convert_label_ht(self,rel_l):
        num_l = len(rel_l)
        rel_flag = 1-self.head_tail[(rel_l+1)]
        #rel_flag = torch.zeros(num_l)
        #rel_flag[torch.where(rel_l > 0)] = 1
        bf_labels = rel_l.new_zeros(num_l,2)
        bf_labels[torch.arange(num_l), rel_flag.long()] = 1
    ...
    def convert_logits_ht(self,relation_logits):
        N,C = relation_logits.shape
        soft_output = F.softmax(relation_logits,dim=1)
        max_value = soft_output.max(1)[1]
        bf_relation_logits = relation_logits.new_zeros(N,2)
        for i in range(len(max_value)):
            if self.head_tail[max_value[i]+1]>0:
                bf_relation_logits[i,0] = relation_logits[i,:].max()
                bf_relation_logits[i,1] = relation_logits[i,(1-self.head_tail).bool()[1:]].mean()
                #bf_relation_logits[i,0] = 0.0 (C-1)
            else:
                bf_relation_logits[i,1] = relation_logits[i,:].max()
                bf_relation_logits[i,0] = relation_logits[i,self.head_tail.bool()[1:]].mean()
        return bf_relation_logits
    ...
    def __call__(self, proposals, rel_labels, relation_logits, refine_logits):
        ...
        #relation_logits_diff = cat(relation_logits_diff, dim=0)
        ...
        if self.use_label_smoothing:
            ...
            #loss_relation = self.EQLloss(relation_logits,rel_labels.long(), qbj = True)###origin by zhouhao
            #loss_relation = self.foreloss(relation_logits,rel_labels.long())#only foreground by zhouhao
            #loss_relation,loss_ht = self.foreloss(relation_logits_diff,rel_labels.long())
            loss_relation,loss_ht = self.foreloss(relation_logits,rel_labels.long())
            #loss_relation = self.EQLloss(relation_logits,rel_labels.long(),relation_logits_diff, qbj = True)

            #loss_relation = self.criterion_loss(relation_logits, rel_labels.long())
            #loss_relation = self.background_append_loss(relation_logits,rel_labels.long())
            loss_refine_obj = self.criterion_loss(refine_obj_logits, fg_labels.long())
            #loss_relation = self.EQLloss(refine_obj_logits,rel_labels.long(), qbj = True)
        ...
        if self.attri_on:
            ...
            return loss_relation*0.9, loss_refine_obj,loss_bf,loss_ht*0.1
